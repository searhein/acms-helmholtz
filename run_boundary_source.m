...
% -div(a*grad(u))-kappa^2*u = 0 in Omega = unit disc,
% a d_n u + i k beta u = g on dOmega = unit circle,
% with boundary source, see Section 5.3.

fprintf('Helmholtz problem with Robin bc on circle and localized boundary source\n');
...
gR=@(p)exp(-200*((p(1,:)+1/sqrt(2)).^2+(p(2,:)-1/sqrt(2)).^2)); % boundary data
f = @(p,t,u,time) 0*p(1,:); % volume data

no_p_FEM = 4e5; % number of vertices for the FEM grid to realize ACMS method
...
%% ACMS method
fprintf("\n Do ACMS method\n")
no_edge_modes = 2.^(1:8); % number of edge modes per edge
no_bubbles = 0; % number of bubble functions per subdomain
